...
struct ClockwiseWebServer
{
      ...
      while (client.connected())
      {
        if (client.available())
        {
          ...
          if (c == '\n')
          {
            ...
            // Serial.print("httpBuffer : " + httpBuffer);
            uint8_t path_pos = httpBuffer.indexOf(' ', method_pos + 1);
            // Serial.print(httpBuffer);
            ...
            String postString = client.readString();

            // Serial.print("postString : " + postString);
            if (path.indexOf('?') > 0)
            {
  ...
  void processRequest(WiFiClient client, String method, String path, String key, String value)
  {
    ...
    else if (method == "POST" && path == "/set")
    {
      ...
      {
        ClockwiseParams::getInstance()->swapRGB = value.toInt();
      }
      ...
      else if (key == ClockwiseParams::getInstance()->PREF_CLOCK_PHASE)
      {
        ClockwiseParams::getInstance()->clockPhase = (value == "1");
      }
      else if (key == ClockwiseParams::getInstance()->THEME_NAME)
      {
  ...
  void getCurrentSettings(WiFiClient client)
  {
    ...
    client.printf(HEADER_TEMPLATE_D, ClockwiseParams::getInstance()->PREF_SWAP_BLUE_GREEN, ClockwiseParams::getInstance()->swapRGB);
    ...
    client.printf(HEADER_TEMPLATE_D, ClockwiseParams::getInstance()->PREF_CLOCK_PHASE, ClockwiseParams::getInstance()->clockPhase);
    client.printf(HEADER_TEMPLATE_S, ClockwiseParams::getInstance()->THEME_NAME, ClockwiseParams::getInstance()->themeName);
    client.printf(HEADER_TEMPLATE_S, ClockwiseParams::getInstance()->THEME_JSON, ClockwiseParams::getInstance()->themeJson);
